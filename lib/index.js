(function() {
  // Package imports
  // TODO: This currently trusts that the packages are required by package.json
  // Would be much better for it to manage its own packages;
  var Q = require('q');
  ...
  getLangResource = (function() {
    ...
    getResourceFile = function(filePath) {
      ...
      if (path.extname(filePath) === '.json') {
        try {
          res = JSON.parse(fs.readFileSync(filePath).toString());
        } catch (_error) {
          e = _error;
        ...
        if (fs.statSync(langDir).isDirectory()) {
          ...
          return async.each(fileList, function(filePath, cb) {
            if (path.extname(filePath) === '.json') {
              ...
              res[path.basename(filePath).replace(/\.json$/, '')] =
